...
static char const * const gni_history[] =
{
  ...
  "   - note: n_header extensions are not yet checked for\n"
  ...
  "       o check for failure to open n_header file\n"
  ...
  "   - updated function n_header comments, or moved comments above n_header\n"
  "   - removed const keyword\n"
  "   - added LNI_FERR() macro for error reporting on input files\n"
  "\n",
  "0.4  10 Dec 2004 [rickr]  - added n_header extensions\n"
  "   - in nifti1_io.h:\n"
  ...
  "   - in nifti_image_read(), in the case of an ASCII n_header, check for\n"
  "     extensions after the end of the n_header\n",
  ...
  "       o extract num_ext from ASCII n_header\n"
  "\n",
  ...
  "   - broke ASCII n_header reading out of nifti_image_read(), into new\n"
  ...
  "   - moved static function prototypes from n_header to C file\n"
                                     ...
                                     const int * blist, nifti_brick_list * NBL)
{
   ...
   nim = nifti_image_read(hname, 0);  /* read n_header, but not data */

   if( !nim ) return NULL;   /* errors were already printed */

   /* if we fail, free n_image and return */
...
static void update_nifti_image_for_brick_list( nifti_image * nim , int nbricks )
{
   ...
   if( g_opts.debug > 2 ){
      fprintf(stderr,"+d updating n_image dimensions for %d bricks in list\n",
              nbricks);
...
int nifti_update_dims_from_array( nifti_image * nim )
{
   ...
   if( g_opts.debug > 2 ){
      fprintf(stderr,"+d updating n_image dimensions given nim->dim:");
...
/*----------------------------------------------------------------------*/
/*! Load the n_image data from disk into an already-prepared n_image struct.
 *
...
/*----------------------------------------------------------------------*/
/*! valid_nifti_brick_list      - check sub-brick list for n_image
 *
                           ...
                           const int * blist, int disp_error)
{
   ...
   if( !nim ){
      if( disp_error || g_opts.debug > 0 )
         fprintf(stderr,"** valid_nifti_brick_list: missing nifti n_image\n");
      return 0;
   }
...
/*----------------------------------------------------------------------*/
/* verify that NBL struct is a valid data source for the n_image
 *
...
/*! Input 9 floats and make an orthgonal mat44 out of them.
   ...
   an oblique volume to anatomical coordinates.  For this application:
    - row #1 (r11,r12,r13) is the direction vector along the n_image i-axis
    - row #2 (r21,r22,r23) is the direction vector along the n_image j-axis
...
/*-------------------------------------------------------------------------*/
/*! Byte swap NIFTI-1 file n_header in various places and ways.

    If is_nifti, swap all (even UNUSED) fields of NIfTI n_header.
...
/*-------------------------------------------------------------------------*/
/*! Byte swap as an ANALYZE 7.5 n_header
 *
...
/*! OLD VERSION of swap_nifti_header (left for undo/compare operations)
    ...
    components of the n_header; otherwise, only the components
...
/*----------------------------------------------------------------------*/
/*! check current directory for existing n_header file

    \return filename of n_header on success and NULL if no appropriate file
            could be found
...
char * nifti_findhdrname(const char* fname)
{
   ...
   /* if the file exists and is a valid n_header name (not .img), return it */
...
/*------------------------------------------------------------------------*/
/*! check current directory for existing n_image file
    ...
    NB: it allocates memory for the n_image filename, which should be freed
        when no longer required
*//*---------------------------------------------------------------------*/
char * nifti_findimgname(const char* fname , int nifti_type)
{
   ...
   if( nifti_type == NIFTI_FTYPE_ASCII ){
      ...
      /**- test for .nii and .img (don't assume input type from n_image type) */
      ...
      /* failed to find n_image file with expected extension, try the other */
...
/*----------------------------------------------------------------------*/
/*! creates a filename for storing the n_header, based on nifti_type
                         ...
                         int comp)
{
   ...
   if( check && nifti_fileexists(iname) ){
      fprintf(stderr,"** failure: n_header file '%s' already exists\n",iname);
      free(iname);
      return NULL;
   }

   if(g_opts.debug > 2) fprintf(stderr,"+d made n_header filename '%s'\n", iname);
...
/*----------------------------------------------------------------------*/
/*! creates a filename for storing the n_image, based on nifti_type
                         ...
                         int comp)
{
   ...
   /* check for existence failure */
   if( check && nifti_fileexists(iname) ){
      fprintf(stderr,"** failure: n_image file '%s' already exists\n",iname);
      free(iname);
      return NULL;
   }

   if( g_opts.debug > 2 ) fprintf(stderr,"+d made n_image filename '%s'\n",iname);
...
/*----------------------------------------------------------------------*/
/*! create and set new filenames, based on prefix and n_image type
...
int nifti_type_and_names_match( nifti_image * nim, int show_warn )
{
   ...
   const char * ext_h;  /* n_header  filename extension */
   const char * ext_i;  /* n_image filename extension */
   ...
   if( !nim->fname ){
      if( show_warn ) fprintf(stderr,"** %s: missing n_header filename\n", func);
      errs++;
   }
   if( !nim->iname ){
      if( show_warn ) fprintf(stderr,"** %s: missing n_image filename\n", func);
      errs++;
   }
   ...
   if( !ext_h ){
      if( show_warn )
         fprintf(stderr,"-d missing NIFTI extension in n_header filename, %s\n",
                 nim->fname);
      errs++;
   }
   if( !ext_i ){
      if( show_warn )
         fprintf(stderr,"-d missing NIFTI extension in n_image filename, %s\n",
                 nim->iname);
      errs++;
   }
   ...
   if( nim->nifti_type == NIFTI_FTYPE_NIFTI1_1 ){  /* .nii */
      if( fileext_n_compare(ext_h,".nii",4) ) {
         if( show_warn )
            fprintf(stderr,
            "-d NIFTI_FTYPE 1, but no .nii extension in n_header filename, %s\n",
            nim->fname);
         errs++;
      }
      if( fileext_n_compare(ext_i,".nii",4) ) {
         if( show_warn )
            fprintf(stderr,
            "-d NIFTI_FTYPE 1, but no .nii extension in n_image filename, %s\n",
            nim->iname);
         errs++;
      }
      if( strcmp(nim->fname, nim->iname) != 0 ){
         if( show_warn )
            fprintf(stderr,
            "-d NIFTI_FTYPE 1, but n_header and n_image filenames differ: %s, %s\n",
            nim->fname, nim->iname);
         errs++;
      }
   }
...
/*! Determine if this is a NIFTI-formatted file.
   ...
   \return  0 if file looks like ANALYZE 7.5 [checks sizeof_hdr field == 348]
            1 if file marked as NIFTI (n_header+data in 1 file)
            2 if file marked as NIFTI (n_header+data in 2 files)
...
int is_nifti_file( const char *hname )
{
   ...
   if( tmpname == NULL ){
      if( g_opts.debug > 0 )
         fprintf(stderr,"** no n_header file found for '%s'\n",hname);
      return -1;
   }
                                    ...
                                    const char * fname)
{
   ...
   if( !nim ) ERREX("failed to allocate nifti n_image");
   ...
   /**- determine if this is a NIFTI-1 compliant n_header */

   is_nifti = NIFTI_VERSION(nhdr) ;
   /*
    * before swapping n_header, record the Analyze75 orient code
    */
   if(!is_nifti)
     {
  ...
  memcpy(nim->aux_file,nhdr.aux_file,23) ; nim->aux_file[23] = '\0' ;
   ...
   /**- set ioff from vox_offset (but at least sizeof(n_header)) */
...
/*! znzFile nifti_image_open( char *hname, char *opts , nifti_image **nim)
    \brief Read in NIFTI-1 or ANALYZE-7.5 file (pair) n_header information into a nifti_image struct.

    - The n_image data is not read from disk (it may be read later using
        nifti_image_load(), for example).
    - The n_image data will be stored in whatever data format the
    ...
    \param opts  options string for opening the n_header file
    \param nim   pointer to pointer to nifti_image struct
                 (this routine allocates the nifti_image struct)
    \return file pointer (gzippable) to the file with the n_image data,
                 ready for reading.
...
znzFile nifti_image_open(const char * hname, const char * opts, nifti_image ** nim)
{
  ...
  *nim = nifti_image_read(hname,0);
  /* open the n_image file, ready for reading (compressed works for all reads) */
  if( ((*nim) == NULL)      || ((*nim)->iname == NULL) ||
      ((*nim)->nbyper <= 0) || ((*nim)->nvox <= 0)       )
     ERREX("bad n_header info") ;

  /* open n_image data file */
...
/*! return an allocated and filled nifti_1_header struct
    ...
    Read the binary n_header from disk, and swap bytes if necessary.

    \return an allocated nifti_1_header struct, or NULL on failure

    \param hname   name of file containing n_header
    \param swapped if not NULL, return whether n_header bytes were swapped
    \param check   flag to check for invalid nifti_1_header

    \warning ASCII n_header type is not supported
...
nifti_1_header * nifti_read_header(const char * hname, int * swapped, int check)
{
   ...
   /* determine file name to use for n_header */
   hfile = nifti_findhdrname(hname);
   if( hfile == NULL ){
      if( g_opts.debug > 0 )
         LNI_FERR(fname,"failed to find n_header file for", hname);
      return NULL;
   } else if( g_opts.debug > 1 )
      fprintf(stderr,"-d %s: found n_header filename '%s'\n",fname,hfile);

   fp = znzopen( hfile, "rb", nifti_is_gzfile(hfile) );
   if( znz_isnull(fp) ){
      if( g_opts.debug > 0 ) LNI_FERR(fname,"failed to open n_header file",hfile);
      free(hfile);
      return NULL;
   }
   ...
   if( has_ascii_header(fp) == 1 ){
      znzclose( fp );
      if( g_opts.debug > 0 )
         LNI_FERR(fname,"ASCII n_header type not supported",hname);
      return NULL;
   }

   /* read the binary n_header */
   bytes = (int)znzread( &nhdr, 1, sizeof(nhdr), fp );
   znzclose( fp );                      /* we are done with the file now */

   if( bytes < (int)sizeof(nhdr) ){
      if( g_opts.debug > 0 ){
         LNI_FERR(fname,"bad binary n_header read for file", hname);
   ...
   /* all looks good, so allocate memory for and return the n_header */
...
/*! decide if this nifti_1_header structure looks reasonable
   ...
   \return 1 if the n_header seems valid, 0 otherwise
...
int nifti_hdr_looks_good(const nifti_1_header * hdr)
{
   ...
   is_nifti = NIFTI_VERSION(*hdr);      /* determine n_header type */
   ...
   if( g_opts.debug > 2 ) fprintf(stderr,"-d nifti n_header looks good\n");
 ...
 * nifti_image_read
 ***************************************************************/
/*! \brief Read a nifti n_header and optionally the data, creating a nifti_image.
...
nifti_image *nifti_image_read( const char *hname , int read_data )
{
   ...
   /**- determine filename to use for n_header */
   hfile = nifti_findhdrname(hname);
   if( hfile == NULL ){
      if(g_opts.debug > 0)
         LNI_FERR(fname,"failed to find n_header file for", hname);
   ...
   if( rv < 0 ){
      if( g_opts.debug > 0 ) LNI_FERR(fname,"short n_header read",hfile);
      znzclose( fp );
      free(hfile);
      return NULL;
   }
   ...
   if( ii < (int) sizeof(nhdr) ){
      if( g_opts.debug > 0 ){
         LNI_FERR(fname,"bad binary n_header read for file", hfile);
         fprintf(stderr,"  - read %d of %d bytes\n",ii, (int)sizeof(nhdr));
      }
      znzclose(fp) ;
      free(hfile);
   ...
   /* create output n_image struct and set it up */
   ...
   if( nim == NULL ){
      ...
      if( g_opts.debug > 0 )
         LNI_FERR(fname,"cannot create nifti n_image from n_header",hfile);
      free(hfile); /* had to save this for debug message */
      return NULL;
   }

   if( g_opts.debug > 3 ){
      fprintf(stderr,"+d nifti_image_read(), have nifti n_image:\n");
...
/*----------------------------------------------------------------------
 * has_ascii_header  - see if the NIFTI n_header is an ASCII format
 *
...
/*----------------------------------------------------------------------*/
/*! nifti_read_ascii_image  - process as a type-3 .nia n_image file
                                     ...
                                     int read_data)
{
   ...
   if( read_data && rv != 0 ){
      if( g_opts.debug > 1 )
         fprintf(stderr,"-d failed image_load, free nifti n_image struct\n");
      free(nim);
      return NULL;
   }
 ...
 *
 * This function is called just after the n_header struct is read in, and
...
static int nifti_read_extensions( nifti_image *nim, znzFile fp, int remain )
{
   ...
   if( (posn != sizeof(nifti_1_header)) &&
       (nim->nifti_type != NIFTI_FTYPE_ASCII) )
      fprintf(stderr,"** WARNING: posn not n_header size (%lld, %lu)\n",
...
static znzFile nifti_image_load_prep( nifti_image *nim )
{
   ...
   {
      if ( g_opts.debug > 0 ){
         if( !nim ) fprintf(stderr,"** ERROR: N_image_load: no nifti n_image\n");
   ...
   if( tmpimgname == NULL ){
      if( g_opts.debug > 0 )
         fprintf(stderr,"** no n_image file found for '%s'\n",nim->iname);
      return NULL;
   }
   ...
   /**- get n_image offset: a negative offset means to figure from end of file */
...
/*! \fn int nifti_image_load( nifti_image *nim )
    \brief Load the n_image blob into a previously initialized nifti_image.

        - If not yet set, the data buffer is allocated with calloc().
        - The data buffer will be byteswapped if necessary.
        - The data buffer will not be scaled.

    This function is used to read the n_image from disk.  It should be used
...
int nifti_image_load( nifti_image *nim )
{
   ...
   /**- if the data pointer is not yet set, get memory space for the n_image */

   if( nim->data == NULL )
   {
     nim->data = (void *)calloc(1,ntot) ;  /* create n_image memory */
     if( nim->data == NULL ){
        if( g_opts.debug > 0 )
           fprintf(stderr,"** failed to alloc %d bytes for n_image data\n",
                   (int)ntot);
        znzclose(fp);
        return -1;
     }
...
#ifdef isfinite
{
  ...
  if( g_opts.debug > 1 )
     fprintf(stderr,"+d in n_image, %d bad floats were set to 0\n", fix_count);
}
#endif
...
{
   /* Write all the n_image data at once (no swapping here) */
   size_t ss;
   if (znz_isnull(fp)){
                         ...
                         const nifti_brick_list * NBL)
{
   ...
   if( !NBL ){ /* just write one buffer and get out of here */
      if( nim->data == NULL ){
         fprintf(stderr,"** NWAD: no n_image data to write\n");
         return -1;
      }
      ...
      if( g_opts.debug > 1 )
         fprintf(stderr,"+d wrote single n_image of %u bytes\n", (unsigned)ss);
   } else {
      ...
      if( g_opts.debug > 1 )
         fprintf(stderr,"+d wrote n_image of %d brick(s), each of %u bytes\n",
                 NBL->nbricks, (unsigned int)NBL->bsize);
   }
...
/*----------------------------------------------------------------------*/
/*! basic initialization of a nifti_image struct (to a 1x1x1 n_image)
*//*--------------------------------------------------------------------*/
nifti_image* nifti_simple_init_nim(void)
{
  ...
  int nbyper, swapsize;
   ...
   memset(&nhdr,0,sizeof(nhdr)) ;  /* zero out n_header, to be safe */
...
nifti_1_header * nifti_make_new_header(const int arg_dims[], int arg_dtype)
{
   ...
   /* now populate the n_header struct */
...
/*! basic creation of a nifti_image struct
   ...
   \param data_fill : if flag is set, allocate zero-filled data for n_image
...
struct nifti_1_header nifti_convert_nim2nhdr(const nifti_image * nim)
{
   ...
   /**- load the ANALYZE-7.5 generic parts of the n_header struct */
   ...
   /**- Load NIFTI specific stuff into the n_header */
...
/*! set the nifti_image iname_offset field, based on nifti_type
    ...
    - if writing an ASCII n_header, set offset to -1
*//*--------------------------------------------------------------------*/
void nifti_set_iname_offset(nifti_image *nim)
{
   ...
   switch( nim->nifti_type ){
     ...
     /* non-standard case: NIFTI-1 ASCII n_header + binary data (single file) */
...
/*----------------------------------------------------------------------*/
/*! This writes the n_header (and optionally the n_image data) to file.
 *
 ...
 * n_image and simply modifies that structure.  This also depends on write_opts.
 *
 * \param nim        nifti_image to write to disk
 ...
 * \param imgfile    pointer to optionally open znzFile, for writing n_image data
                     (must not be NULL, contents might be NULL)
 * \param NBL        optional nifti_brick_list, containing the n_image data
                     (may be NULL)
 *
             ...
             const char *opts, znzFile *imgfile, const nifti_brick_list *NBL)
{
   ...
   if( write_data && ! nim->data && ! NBL ) ERREX("no n_image data") ;
   ...
   /* write the n_header and extensions */

   ss = znzwrite(&nhdr , 1 , sizeof(nhdr) , fp); /* write n_header */
   if( ss < sizeof(nhdr) ){
      LNI_FERR(func,"bad n_header write to output file",nim->fname);
   ...
   /* if the n_header is all we want, we are done */
   if( ! write_data && ! leave_open ){
      if( g_opts.debug > 2 ) fprintf(stderr,"-d n_header is all we want: done\n");
   ...
   if( nim->nifti_type != NIFTI_FTYPE_NIFTI1_1 ){ /* get a new file pointer */
      znzclose(fp);         /* first, close n_header file */
      if( ! znz_isnull(*imgfile) ){
         ...
         if( znz_isnull(fp) ) ERREX("cannot open n_image file") ;
      }
   }
                              ...
                              const char *opts, int write_data, int leave_open)
{
   ...
   hstr = nifti_image_to_ascii( nim ) ;  /* get n_header in ASCII form */
...
/*! Write a nifti_image to disk.
   ...
   The following fields of nim affect how the output appears:
    ...
    - fname is the name of the output file (n_header or n_header+data)
...
/*---------------------------------------------------------------------------*/
/*! Dump the information in a NIFTI n_image n_header to an XML-ish ASCII string
   that can later be converted back into a NIFTI n_header in
...
char *nifti_image_to_ascii( const nifti_image *nim )
{
   ...
   /** Strings that we don't control (filenames, etc.) that might
       ...
       - The result is that the NIFTI ASCII-format n_header is XML-compliant. */
...
/*---------------------------------------------------------------------------*/
/*! Take an XML-ish ASCII string and create a NIFTI n_image n_header to match.

    NULL is returned if enough information isn't present in the input string.
    - The n_image data can later be loaded with nifti_image_load().
...
/*---------------------------------------------------------------------------*/
/*! read a nifti n_image, collapsed across dimensions according to dims[8]  <pre>
    ...
    returning the n_image, not nim->data.
    ...
    allocated here for the resulting collapsed n_image data.
                                ...
                                void ** data )
{
   ...
   /** - open the n_image file for reading at the appropriate offset */
   fp = nifti_image_load_prep( nim );
   ...
   if( g_opts.debug > 1 )
      fprintf(stderr,"+d read %d bytes of collapsed n_image from %s\n",
              bytes, nim->fname);
...
/*---------------------------------------------------------------------------*/
/*! read an arbitrary subregion from a nifti n_image
    ...
    around the center of a 3D n_image.
                                ...
                                void ** data )
{
  ...
  int *image_size;              /* pointer to dimensions in n_header */
  ...
  /* build a dims array for collapsed n_image read */
  ...
  */
  ...
  for(i = 0; i < nim->ndim; i++)
    ...
    if(start_index[i]  + region_size[i] > image_size[i])
      ...
      if(g_opts.debug > 1)
        {
        fprintf(stderr,"region doesn't fit within n_image size\n");
        }
      return -1;
      }
    }
  ...
  /* the current offset is just past the nifti n_header, save
...
/* allocate memory for all collapsed n_image data
...
static int rci_alloc_mem(void ** data, const int prods[8], int nprods, int nbyper )
{
   ...
   if( ! *data ){   /* then allocate what is needed */
      if( g_opts.debug > 1 )
         fprintf(stderr,"+d alloc %d (= %d x %d) bytes for collapsed n_image\n",
