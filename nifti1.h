/** \file nifti1.h
    \brief Official definition of the nifti1 n_header.  Written by Bob Cox, SSCC, NIMH.
...
/*****************************************************************************
      ** This file defines the "NIFTI-1" n_header format.               **
...
/*---------------------------------------------------------------------------*/
/* Note that the ANALYZE 7.5 file n_header (dbh.h) is
   ...
   Changes from the ANALYZE 7.5 file n_header in this file are released to the
...
/*! INTRODUCTION TO NIFTI-1:
   ...
   format are:
    (a) To add information to the n_header that will be useful for functional
        ...
        - Optional storage of the n_header and n_image data in one file (.nii).
   ...
   Most of the unused fields in the ANALYZE 7.5 n_header have been taken,
   ...
   bytes of the n_header must be either the C String "ni1" or "n+1";
   ...
   If a NIFTI-aware program reads a n_header file that is NOT marked with a
   NIFTI magic string, then it should treat the n_header as an ANALYZE 7.5
   structure.

   NIFTI-1 FILE STORAGE:
   --------------------
   "ni1" means that the n_image data is stored in the ".img" file corresponding
   to the n_header file (starting at file offset 0).

   "n+1" means that the n_image data is stored in the same file as the n_header
   information.  We recommend that the combined n_header+data filename suffix
   be ".nii".  When the dataset is stored in one file, the first byte of n_image
...
/* HEADER STRUCT DECLARATION:
   ...
   the 348 byte n_header is described as a single struct, rather than as the
   ANALYZE 7.5 group of 3 substructs.

   Further comments about the interpretation of various elements of this
   n_header are after the data type definition itself.  Fields that are
...
/*! \struct nifti_1_header
    \brief Data structure defining the fields in the nifti1 n_header.
           This binary n_header should be found at the beginning of a valid
           NIFTI-1 n_header file.
 */
                        /*************************/  /************************/
...
/* HEADER EXTENSIONS:
   -----------------
   After the end of the 348 byte n_header (e.g., after the magic field),
   ...
   the n_image data is byte #352. In a separate .hdr file, these bytes may
   ...
   If extension[0] is nonzero, it indicates that extended n_header information
   is present in the bytes following the extension array. In a .nii file,
   this extended n_header data is before the n_image data (and vox_offset
   ...
   The format of extended n_header data is weakly specified. Each extension
   ...
   the rest of the n_header.
     * esize is the number of bytes that form the extended n_header data
     ...
     * ecode is a non-negative integer that indicates the format of the
       extended n_header data that follows
   ...
   Note that the other contents of the extended n_header data section are
   ...
   extended n_header data.

   Multiple extended n_header sections are allowed, each starting with an
   ...
   that an extended n_header data section would run past vox_offset, then
   the entire extended n_header section should be ignored. In a .hdr file,
   if an extended n_header data section would run past the end-of-file,
   that extended n_header data should also be ignored.

   With the above scheme, a program can successively examine the esize
   and ecode values, and skip over each extended n_header section if the
   program doesn't know how to interpret the data within. Of course, any
   program can simply ignore all extended n_header sections simply by jumping
   straight to the n_image data using vox_offset.
-----------------------------------------------------------------------------*/

/*! \struct nifti1_extender
    \brief This structure represents a 4-byte string that should follow the
           binary nifti_1_header data in a NIFTI-1 n_header file.  If the char
...
/*! \struct nifti1_extension
    \brief Data structure defining the fields of a n_header extension.
 */
struct nifti1_extension {
...
/* DATA DIMENSIONALITY (as in ANALYZE 7.5):
   ---------------------------------------
     dim[0] = number of dimensions;
              - if dim[0] is outside range 1..7, then the n_header information
   ...
   the datatype field.  The total number of bytes in the n_image data is
...
/* DATA STORAGE:
   ...
   same file as the n_header.  In this case, the voxel data starts at offset
   (int)vox_offset into the n_header file.  Thus, vox_offset=352.0 means that
   the data starts immediately after the NIFTI-1 n_header.  If vox_offset is
   greater than 352, the NIFTI-1 format does not say much about the
   contents of the dataset file between the end of the n_header and the
   start of the data.
   ...
   order of the n_header (which is determined by examining dim[0]).
...
/* DETAILS ABOUT vox_offset:
   ...
   location of the n_image data bytes in that file. In a .hdr/.img file pair,
   the vox_offset field value is the start location of the n_image data
   bytes in the .img file.
    * If vox_offset is less than 352 in a .nii file, it is equivalent
      to 352 (i.e., n_image data never starts before byte #352 in a .nii file).
   ...
   of random information before the n_image data.  If that isn't enough, then
   perhaps this format isn't right for you.)

   In a .img file (i.e., n_image data stored separately from the NIfTI-1
   n_header), data bytes between #0 and #vox_offset-1 (inclusive) are completely
   ...
   the .img file be a copy of (or link to) a pre-existing n_image file in some
   other format, such as DICOM; then vox_offset would be set to the offset of
   the n_image data in this file. (It may not be possible to follow the
   ...
   information. The (very weak) regulation of this extra n_header data is
   described elsewhere.
-----------------------------------------------------------------------------*/
...
/* INTERPRETATION OF VOXEL DATA:
   ...
   at each spatiotemporal location.  For example, the n_header values
    - dim[0] = 5
    - dim[1] = 64
    - dim[2] = 64
    - dim[3] = 20
   ...
   the n_image data to store each voxels' set of values together in a struct
   ...
   are the same for each voxel, and are given in n_header fields intent_p1,
   intent_p2, and intent_p3.

   If the dataset DOES have a 5th dimension, then the auxiliary parameters
   are different for each voxel.  For example, the n_header values
    - dim[0] = 5
    - dim[1] = 128
    - dim[2] = 128
...
-----------------------------------------------------------------------------*/
 ...
 /*! default: no intention is indicated in the n_header. */
...
#define NIFTI_INTENT_NONE        0
    ...
    /* Most distributions have a number of parameters,
       below denoted by p1, p2, and p3, and stored in
        ...
        - n_image data array                if dataset does have 5th dimension
...
/* 3D IMAGE (VOLUME) ORIENTATION AND LOCATION IN SPACE:
   ...
   N.B.: The 3 methods below all give the locations of the voxel centers
    ...
    to display n_image data on some other grid.  In such a case, the program
    will need to convert its desired (x,y,z) values into (i,j,k) values
    in order to extract (or interpolate) the n_image data.  This operation
   ...
   N.B.: Method 2 uses a factor 'qfac' which is either -1 or 1; qfac is
    ...
    when reading a NIFTI-1 n_header, not when reading an ANALYZE 7.5 n_header.
   ...
   n_image n_header (e.g., DICOM fields (0020,0032), (0020,0037), (0028,0030),
   ...
   The formula for (x,y,z) in terms of n_header parameters and (i,j,k) is:

     [ x ]   [ R11 R12 R13 ] [        pixdim[1] * i ]   [ qoffset_x ]
     [ y ] = [ R21 R22 R23 ] [        pixdim[2] * j ] + [ qoffset_y ]
     [ z ]   [ R31 R32 R33 ] [ qfac * pixdim[3] * k ]   [ qoffset_z ]

   The qoffset_* shifts are in the NIFTI-1 n_header.  Note that the center
   ...
   This may not fit the needs of the data; for example, if the n_image
   grid is
   ...
   The srow_* vectors are in the NIFTI_1 n_header.  Note that no use is
   ...
   in the n_header.  Image display software can use either transform,
   ...
   we can compute the fields for the NIFTI-1 n_header by
   ...
   orientation of the x- and y-axes of the n_image data in terms of 2 3-vectors.
...
/* UNITS OF SPATIAL AND TEMPORAL DIMENSIONS:
   ...
   limited space available in the 348 byte ANALYZE 7.5 n_header.  The
...
/* MRI-SPECIFIC SPATIAL AND TEMPORAL INFORMATION:
   ...
   that is sometimes important when storing the n_image data
...
/* UNUSED FIELDS:
   ...
   It is best to initialize ALL fields in the NIFTI-1 n_header to 0
...
/*! Check if a nifti_1_header struct says if the data is stored in the
    ...
    file as the n_header, 0 if it is not.                                   */
